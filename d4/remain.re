...
let find_x_mas = lines_of_2 =>
  List.fold_left((acc, l) => acc * find_sam(l), 1, lines_of_2);
  ...
  List.iteri(
    (x, l) =>
      List.iteri(
        (y, _) => {
          let cur_pos = (x, y);
          let lines_of_2 = get3len_strs(all_lines^, valid_2, cur_pos);
          let local = find_x_mas(lines_of_2);
          count := count^ + local;
        },
        List.of_seq @@ String.to_seq(l),
      ),
